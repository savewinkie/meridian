...
export default function UploadScannerPage() {
  ...
  useEffect(() => {
    setFreeUsed(localStorage.getItem("qualix_free_upload") === "1")
  ...
  async function scan() {
    ...
    try {
      ...
      if (isLoggedIn === false) {
        localStorage.setItem("qualix_free_upload", "1")
        setFreeUsed(true)
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#060b16] overflow-y-auto">
      ...
      <div className={cn("flex flex-col flex-1 gap-5 p-6 items-center", !results ? "justify-center" : "")}>
